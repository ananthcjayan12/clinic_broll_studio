...
def render(run_id: str, mode: str, output: Path) -> dict[str, Any]:
    with _render_lock(output):
        ...
        worker_args = _capture_worker_args(composition)
        ...
        if worker_args:
            ...
            base_command.extend(worker_args)
            ...
            append_log(run_paths(run_id), "HyperFrames: alpha composition using 6 partitioned capture workers")
        attempt = _next_attempt(output)
        log = _attempt_log(output, attempt)
        result = _run_streaming(run_id, base_command, composition.parent, log, env=render_env)
        ...
        return {"composition": str(composition), "output": str(output), "log": str(log), "logs": [str(log)]}


def _capture_worker_args(composition: Path) -> list[str]:
