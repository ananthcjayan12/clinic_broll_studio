...
def run(run_id: str) -> dict[str, Any]:
    ...
    selected_slots = [
        slot for slot in plan.get("slots", [])
        if slot.get("selected_motion") or slot.get("selected_still")
    ]
    windows = _chunk_windows(duration, fps, selected_slots)
    overlay_dir = _next_overlay_dir(paths.renders)
    overlay_dir.mkdir(parents=True)
    append_log(paths, f"Final render: composition valid; rendering {len(windows)} transparent chunks")
    rendered_chunks = []
    chunk_files = []
    for index, window in enumerate(windows, start=1):
        chunk = overlay_dir / f"part-{index:03d}.webm"
        chunk_files.append(chunk)
        append_log(paths, f"Final render: transparent chunk {index}/{len(windows)}")
    ...
    concat_result = subprocess.run(
        _concat_command(concat_manifest, overlay), capture_output=True, text=True, timeout=14400
    )
    if concat_result.returncode != 0:
        raise RuntimeError(f"FFmpeg overlay concat failed: {(concat_result.stderr or concat_result.stdout)[-5000:]}")
    append_log(paths, "Final render: compositing overlay over the full-resolution master")
    command = _composite_command(paths.source / "master.mp4", overlay, candidate_output)
    ...
    if archived_final is not None:
        append_log(paths, f"Final render: previous MP4 preserved as {archived_final.name}")
    ...
    return {
        ...
        "summary": {"validation": validation, "render_chunks": rendered_chunks, "windows": windows},
    }
...
def _chunk_windows(
    ...
    """Create frame-aligned windows without placing a boundary inside a slot."""
    ...
    intervals = sorted(
        (float(slot["start"]), float(slot["start"]) + float(slot["duration"]))
        for slot in slots
    )
        ...
        if end_frame < total_frames:
            ...
            if crossing:
                ...
                if safe_before <= start_frame:
                    raise ValueError("A selected B-roll slot is longer than the maximum render chunk")
                end_frame = safe_before
        frame_windows.append((start_frame, end_frame))
        start_frame = end_frame
    return [
        (start / fps, duration if end == total_frames else end / fps)
...
def _composite_command(master: Path, overlay: Path, output: Path) -> list[str]:
    return [
        FFMPEG,
        "-hide_banner",
        "-n",
        ...
        str(overlay),
        "-filter_complex",
        "[0:v:0][1:v:0]overlay=0:0:format=auto:eof_action=pass[v]",
        "-map",
        "[v]",
        "-map",
        "0:a?",
