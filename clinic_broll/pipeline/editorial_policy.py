...
def apply_editorial_policy(plan: dict[str, Any], settings: dict[str, Any]) -> dict[str, Any]:
    ...
    # Director beats can legitimately span a whole explanation.  Treating an
    # 8–12 second visual beat as one uninterrupted B-roll clip caused the
    # consecutive-visual guard below to discard it entirely.  Split only the
    # editorial treatment (never the source timing): retain a short, useful
    # visual segment and return the remaining source range to the doctor.
    scenes = _cap_visual_beat_duration(scenes, maximum_seconds=6.0)
    ...
    # Every visual strategy is now generated by the configured image model.
    # Keep the approved request count inside the operator's paid-generation
    # budget rather than treating diagrams or editorial graphics as free.
    configured_candidates = max(1, min(int(settings.get("image_candidates_per_slot") or 1), 3))
    max_generated_scenes = budget["max_initial_generations"] // configured_candidates
    generated_scenes = sorted(
        [scene for scene in scenes if scene.get("visual_strategy") != "none"],
        key=_scene_score,
        reverse=True,
    )
    for scene in generated_scenes[max_generated_scenes:]:
        _make_talking_head(scene)
...
def budget_report(
    ...
    illustrations = [scene for scene in visual if scene.get("visual_strategy") in {"dental_diagram", "editorial_graphic"}]
    ...
    expected = len(visual) * candidates
    violations: list[str] = []
    ...
    return {
        ...
        "generated_illustration_scenes": len(illustrations),
        "local_graphic_scenes": 0,
...
def _cap_visual_beat_duration(scenes: list[dict[str, Any]], *, maximum_seconds: float) -> list[dict[str, Any]]:
    capped: list[dict[str, Any]] = []
    for scene in scenes:
        duration = float(scene.get("duration") or (float(scene.get("end") or 0) - float(scene.get("start") or 0)))
        if scene.get("visual_strategy") == "none" or duration <= maximum_seconds:
            capped.append(scene)
            continue
        visual_end = round(float(scene["start"]) + maximum_seconds, 3)
        visual_scene = dict(scene)
        visual_scene.update({"end": visual_end, "duration": round(visual_end - float(scene["start"]), 3)})
        talking_head_remainder = dict(scene)
        talking_head_remainder.update({"start": visual_end, "duration": round(float(scene["end"]) - visual_end, 3)})
        _make_talking_head(talking_head_remainder)
        capped.extend([visual_scene, talking_head_remainder])
    return capped
